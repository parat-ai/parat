...
Constraints:
  ...
  - Ticket 1 (scaffold) is a prerequisite — layout.tsx and globals.css
    must exist before this ticket is executed

Existing Code to Reuse:
  - src/styles/globals.css (created in ticket 1, prerequisite)
      Modify: add @import './tokens.css' BEFORE @tailwind directives
...
Components:
  - TokenFile: src/styles/tokens.css — sole owner of all design tokens,
      structured in four CSS blocks (see layout below)
  - GlobalsImport: src/styles/globals.css modification — @import './tokens.css'
      placed at top of file, before @tailwind base/components/utilities
  - SmokeTestPage: src/app/test-tokens/page.tsx — temporary visual
      verification page (deleted after ticket review, not merged to main)
...
Interfaces:
  ...
  Fallback mechanism (applied to every OKLCH token):
    ...
    Block 2 — @supports (color: oklch(0% 0 0)) {
      ...
      @media (prefers-color-scheme: dark) {
        :root { OKLCH dark overrides }      ← dark + OKLCH
      }
    }
    Block 3 — @media (prefers-color-scheme: dark) {
      :root { HSL dark values }             ← dark, non-OKLCH browsers
    }
    Block 4 — @media (prefers-reduced-motion: reduce) {
      * { animation-duration: 0.01ms; transition-duration: 0.01ms; }
    }

  Surface semantic token dark-mode resolution:
    Tokens not in the brand spec dark palette are mapped as follows,
    preserving their semantic role (slightly-elevated, muted-fill):
  ...
  Token groups to define (both HSL and OKLCH forms):
...
File/Class Layout:
  ...
  - src/styles/globals.css       modified: @import './tokens.css' added
                                  at top of file
